...
def new_email():
    ...
    email = 'AGAuto-{0}@mii.imgag.com{1}'.format(current_time("%m%d%y%I%M%S"), "hi")
    return email
...
def current_date(date_format):
    ...
    """
    formatted_current_date = datetime.now().strftime(date_format)
...
def browser_exception(test):
    ...
    """
    user_agent = test.browser.execute_script("return navigator.userAgent")
...
def wait_element(test, locator_type, locator_value):
    ...
    try:
        element = WebDriverWait(test.browser, timeout).until(EC.visibility_of_element_located((locator_type, locator_value)))
...
def wait_for_expected_url(test, expected):
   """ Function is used to wait for title to render on page and     
   if text not rendered in 60 seconds, test fail with timeout message.
       @param test: Instance of SeleniumTestScript class. 
       @param text: text on page to verify.  
       @type text: String  
   ...
   for i in range(timeout):
       try:
           if test.browser.current_url == expected: break
       except: pass
       time.sleep(1)
   else: 
       test.fail("Error: Time out, expected url not found.")
